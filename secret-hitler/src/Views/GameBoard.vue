...
import DecksEndTermButton from '../components/DecksEndTermButton.vue'
  ...
  const initializeDeck = () => {
    // 11 fascistas y 6 liberales para 6-7 jugadores
  ...
  // Barajar el mazo
  const shuffleDeck = () => {
    for (let i = policiesDeck.value.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [policiesDeck.value[i], policiesDeck.value[j]] = [policiesDeck.value[j], policiesDeck.value[i]]
    }
  }
  ...
  const drawPolicies = () => {
    if (policiesDeck.value.length < 3) {
      // Si el mazo tiene menos de 3 cartas, barajar los descartes
  ...
  const enactPolicy = (policyType) => {
    ...
    // Añadir las políticas no seleccionadas al descarte
    ...
    // Actualizar el rastreador de elecciones
